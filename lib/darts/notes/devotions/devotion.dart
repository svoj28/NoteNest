...
import 'package:memo_vault/darts/notes/devotions/read/devotionnote_details_screen.dart';
import 'package:memo_vault/darts/notes/devotions/add/devotionnote_add_screen.dart';

class DevotionScreen extends StatefulWidget {
  const DevotionScreen({super.key});

  @override
  _DevotionScreenState createState() => _DevotionScreenState();
}
...
class _DevotionScreenState extends State<DevotionScreen> {
  ...
  void _loadNotes() {
    setState(() {
      _notes = DatabaseHelper().getdevotionNotes();
    });
  }
  ...
  Future<List<Map<String, dynamic>>> _getFilteredNotes() async {
    ...
    if (monthFilter != null) {
      ...
      whereArgs.add(monthFilter.padLeft(2, '0'));
    }
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                await DatabaseHelper()
                    .updateDevotionNotePinned(noteId, !isPinned);
  ...
  Future<void> _showDeleteConfirmation(int noteId) async {
    ...
    if (confirmed == true) {
      await DatabaseHelper().deletedevotionNoteById(noteId);
      _filterNotes(); // Refresh the list
    }
  }
